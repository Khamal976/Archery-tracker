      ...
      <Card title="Древко">
        ...
        {!manual && shaft && (
          <>
            ...
            {/*
              Переменное сечение производитель называет словами, а процент,
              на который снимается статический спайн, не печатает. Считаем без
              поправки, но молчать об этом нельзя: сторона ошибки известна.
            */}
            {shaft.barreled && !shaft.focComp && (
              <div className="mt-2">
                <Note tone="warn">
                  Производитель называет древко бочкообразным, но насколько передок жёстче —
                  не публикует. Расчёт идёт без этой поправки и показывает древко жёстче,
                  чем оно ведёт себя: у похожих древков в базе Стю поправка в 2–5 % стоит
                  2–4 фунтов динамического спайна.
                </Note>
              </div>
            )}
          </>
